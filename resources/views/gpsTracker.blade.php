...
<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <!-- Title -->
            <h1 class="text-3xl font-semibold text-gray-900 dark:text-white mb-8">GPS Tracking</h1>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
                ...
                <div class="p-4 flex justify-between items-center border-b border-gray-200 dark:border-gray-700">
                    <div class="mb-4">
                        <x-link-button-info class="mt-4" href="{{ route('tracking.from-add') }}">
                            {{ __('New vehicle') }}
                        </x-link-button-info>
                    </div>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse ($gpsList as $gps)
                                <tr onclick="toggleDetails({{ $gps->id }})" class="cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                        {{ $gps->vehicle->license_plate }} - {{ $gps->vehicle->type }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                        <div class="flex justify-center items-center gap-2">
                                            <x-link-button-info href="{{ route('tracking.from-edit', $gps->id) }}">
                                                <x-heroicon-o-pencil-square class="w-4 h-4"/>
                                            </x-link-button-info>                                        
                                    
                                            <x-danger-button onclick="confirmDelete({{ $gps->id }})">  
                                                <x-heroicon-o-trash class="w-4 h-4"/>
                                            </x-danger-button>  
                                        </div>
                                    
                                        <form id="delete-form-{{ $gps->id }}" action="{{ route('tracking.delete', $gps->id) }}" method="POST" style="display: none;">  
                                            @csrf  
                                            @method('DELETE')  
                                        </form>  
                                    </td>                                    
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <svg id="icon-{{ $gps->id }}" class="w-5 h-5 transition-transform duration-200 ease-in-out" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                                        </svg>
                                    </td>
                                </tr>

                                <tr id="details-{{ $gps->id }}" class="hidden bg-gray-50 dark:bg-gray-900">
                                    <td colspan="5" class="px-6 py-4">
                                        <div class="mb-4 flex justify-between items-center">
                                            <div>
                                                <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">
                                                    Type of Vehicle: <span class="text-sm">{{ $gps->vehicle->type }}</span>
                                                </h3>
                                                <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">
                                                    Speed of Vehicle: <span class="text-sm">{{ $gps->speed }} Km/H</span>
                                                </h3>
                                            </div>
                                        </div>

                                        <!-- Peta GPS -->
                                        <div id="map-{{ $gps->id }}" class="h-64 w-full rounded-lg shadow"></div>
