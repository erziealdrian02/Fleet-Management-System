<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow" 
                ...
                <div class="p-4 flex justify-between items-center border-b border-gray-200 dark:border-gray-700">
                    <div class="mb-4">
                        @include('formRequest.add-addRequest-form')
                    </div>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody>
                            ...
                            @foreach ($requestList as $request)  
                                <tr onclick="toggleDetails('details-{{ $request->id }}')" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:dark:bg-gray-600 cursor-pointer">
                                    <td class="px-6 py-4 text-center flex items-center justify-center">  
                                        <svg id="icon-{{ $request->id }}" class="transition-transform duration-300 fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">  
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ $request->kode_transaksi }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ $request->vehicle->license_plate }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ $request->vehicle->type }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ $request->tanggal }}</td>   
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                        <x-danger-button type="button" class="ms-3" onclick="confirmDelete({{ $request->id }})">  
                                            <i class="fas fa-trash"></i>  
                                        </x-danger-button>  
                                        
                                        <form id="delete-form-{{ $request->id }}" action="{{ route('requestPart.delete', $request->id) }}" method="POST" style="display: none;">  
                                            @csrf  
                                            @method('DELETE')  
                                        </form>                                              
                                    </td>   
                                ...
                                <!-- Detail Row -->
                                <tr id="details-{{ $request->id }}" class="hidden bg-gray-50 dark:bg-gray-900 ">
                                    <td colspan="7" class="px-6 py-4">
                                        <div class="mb-4 flex justify-between items-center">
                                            <div>
                                                ...
                                                <p class="text-gray-600 dark:text-gray-400">{{ $request->kode_transaksi }}</p>
                                            </div>
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <p class="text-gray-600 dark:text-gray-400">{{ $request->tanggal }}</p>
                                        </div>
                                        <div class="relative overflow-x-auto">
                                            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                                ...
                                                <tbody>
                                                    @foreach ($request->detailRequest as $detail)  
                                                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">  
                                                            <td class="px-6 py-4 text-center">{{ $loop->iteration }}</td>
                                                            <td class="px-6 py-4 text-center">{{ $detail->parts->name }}</td>
                                                            <td class="px-6 py-4 text-center">{{ $detail->quantity }}</td>  
                                                            <td class="px-6 py-4 text-center">Rp. {{ number_format($detail->parts->price, 0, ',', '.') }}</td>
                                                            <td class="px-6 py-4 text-center">Rp. {{ number_format($detail->parts->price * $detail->quantity, 0, ',', '.') }}</td>
                                                        </tr>  
                                                    @endforeach  
                                                </tbody>
                                            </table>
    ...
    <script>
        function sparePartsHandler() {
            return {
                allSpareparts: @json($requestList instanceof \Illuminate\Pagination\LengthAwarePaginator ? $requestList->items() : $requestList),
                isEdit: false,
                searchQuery: '',
                currentPage: 1,
                itemsPerPage: 10,
